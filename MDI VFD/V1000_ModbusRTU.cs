...
namespace V1000_ModbusRTU
{
    ...
    public class V1000_Mon_Data : ICloneable
    {
        ...
        private ushort _MonVal;
        private string _MonValDisp;

        public V1000_Mon_Data()
        {
            RegAddr = 0;
            MonNum = "";
            MonName = "";
        ...
        public V1000_Mon_Data(ushort p_Addr, string p_Num, string p_Name, ushort p_Mult, byte p_Base,  string p_Units, ushort p_Val)
        {
            this.RegAddr = p_Addr;
            this.MonNum = p_Num;
            this.MonName = p_Name;
            this.Multiplier = p_Mult;
            this.NumBase = p_Base;
            this.Units = p_Units;
            this.MonVal = p_Val;
        }

        public ushort MonVal
        {
            get => MonVal;
            set
            {
                ...
                this._MonVal = value;
                if(NumBase == 16)
                    this._MonValDisp = "0x" + this._MonVal.ToString("X4");
                else
                {
                    switch(this.Multiplier)
                    {
                    ...
                    this._MonValDisp = Math.Round(((Double)this._MonVal / this.Multiplier), round_val).ToString() + " " + Units;
                }

            }
        }
        public string MonValDisp => _MonValDisp;

        public object Clone()
        {
            return new V1000_Mon_Data(this.RegAddr, this.MonNum, this.MonName, this.Multiplier, this.NumBase, this.Units, this.MonVal);
        }
    }
}
